...
export async function loadHCAD() {
    ...
    for (let account of hcadMap.values()) {
        ...
        if (i == 50) {
            fs.writeFileSync(`${filePath}/${outputFile}-${i}.json`, JSON.stringify(outputArr))
        }
        if (i % 500000 == 0) {
            fs.writeFileSync(`${filePath}/${outputFile}-${i}.json`, JSON.stringify(outputArr))
            outputArr = []
...
async function getRealAcct() {
    ...
    for await (const row of fileStream) {
        ...
        if (account == "acct" || account == null)
            continue;
        
        const sqFt = parseInt(columns[38], 10) || 0
        const imprvVal = parseInt(columns[44], 10) || 0
        
        const priceSqFt = sqFt > 0 ? 
            parseFloat((imprvVal / sqFt).toFixed(2)) : 0
...
async function getOwners() {
    ...
    for await (const row of fileStream) {
        ...
        if (columns[1] > 1 || account == "acct" || account == null)
            continue;
...
async function getBuildingRes() {
    ...
    for await (const row of fileStream) {
        ...
        if (columns[2].trim() > 1 || account == "acct" || account == null) 
            continue;
        
        const dateBuilt = parseInt(columns[12], 10) || 0
        const yrRemodel = (parseInt(columns[14], 10) || 0) == 0 ? "NA" : parseInt(columns[14], 10)
...
async function getTaxRates() {
    ...
    for await (const row of fileTaxJurisdictions) {
        ...
        if (account == "acct" || account == '' || account == null)
            continue
